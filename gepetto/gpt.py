...
class GPTModelSync():
    ...
    def chat(self, messages, temperature=1.8, model="gpt-4-1106-preview", top_p=0.6):
        """Chat with the model.
    ...
    def function_call(self, messages = [], tools = [], temperature=0.7, model="gpt-4-1106-preview"):
        api_key = os.getenv("OPENAI_API_KEY")
        api_base = "https://api.openai.com/v1/"
        client = OpenAI(api_key=api_key, base_url=api_base)
        response = client.chat.completions.create(
